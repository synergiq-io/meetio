...
async function joinRoom() {
    ...
    try {
        ...
        // Setup presentation listeners
        setupPresentationSocketListeners();
...
const presentPowerPointBtn = document.getElementById('present-powerpoint-btn');
...
const browseBt = document.getElementById('browse-btn');
const closeUploadBtn = document.getElementById('close-upload-btn');
const uploadProgress = document.getElementById('upload-progress');
const progressFill = document.getElementById('progress-fill');
const progressText = document.getElementById('progress-text');

const endPresentationBtn = document.getElementById('end-presentation-btn');
const prevSlideBtn = document.getElementById('prev-slide-btn');
const nextSlideBtn = document.getElementById('next-slide-btn');

// Presentation Event Listeners
if (presentPowerPointBtn) {
    presentPowerPointBtn.addEventListener('click', openPresentationUpload);
}

if (browseBt) {
    browseBt.addEventListener('click', () => pptxFileInput.click());
}

if (closeUploadBtn) {
    closeUploadBtn.addEventListener('click', closePresentationOverlay);
}

if (pptxFileInput) {
    pptxFileInput.addEventListener('change', handleFileSelect);
}

if (uploadArea) {
    uploadArea.addEventListener('click', () => pptxFileInput.click());
...
if (prevSlideBtn) {
    prevSlideBtn.addEventListener('click', () => changeSlide(currentSlideIndex - 1));
}

if (nextSlideBtn) {
    nextSlideBtn.addEventListener('click', () => changeSlide(currentSlideIndex + 1));
}

// Socket Events for Presentation
function setupPresentationSocketListeners() {
    socket.on('presentation-started', (data) => {
        console.log('Presentation started by host');
        showAudienceView(data);
    });

    socket.on('slide-changed', (data) => {
        console.log('Slide changed to:', data.slideIndex);
        updateAudienceSlide(data);
    });
...
// Open presentation upload dialog
function openPresentationUpload() {
    presentationOverlay.classList.remove('hidden');
    presentationUpload.classList.remove('hidden');
...
function closePresentationOverlay() {
    ...
    presentationUpload.classList.add('hidden');
    presenterView.classList.add('hidden');
    audienceView.classList.add('hidden');
...
async function processFiles(files) {
    ...
    try {
        ...
        // For demo, create empty notes array
        const notes = Array(slideCount).fill('');
...
function startPresentation() {
    ...
    // Hide upload, show presenter view
    presentationUpload.classList.add('hidden');
    presenterView.classList.remove('hidden');

    // Notify other participants
    socket.emit('start-presentation', presentationData);
...
function updatePresenterView() {
    ...
    document.getElementById('current-slide-img').src = slide.url;

    // Update next slide preview
    if (currentSlideIndex < totalSlides - 1) {
        document.getElementById('next-slide-img').src = 
            presentationData.slides[currentSlideIndex + 1].url;
    } else {
        document.getElementById('next-slide-img').src = '';
    }

    // Update speaker notes
    const notes = presentationData.notes[currentSlideIndex] || 'No notes for this slide.';
    document.getElementById('speaker-notes-content').textContent = notes;
...
function changeSlide(newIndex) {
    ...
    // Notify participants
    socket.emit('change-slide', currentSlideIndex);
}
...
// Show audience view (participants)
function showAudienceView(data) {
    presentationOverlay.classList.remove('hidden');
    ...
    document.getElementById('audience-slide-counter').textContent = 
        `Slide ${currentSlideIndex + 1} of ${data.totalSlides}`;
    document.getElementById('audience-slide-img').src = data.slides[currentSlideIndex].url;
}

// Update audience slide when host changes
function updateAudienceSlide(data) {
    currentSlideIndex = data.slideIndex;
    document.getElementById('audience-slide-img').src = data.slideUrl;
    document.getElementById('audience-slide-counter').textContent = 
...
document.addEventListener('keydown', (e) => {
    if (!isPresentationActive || !isOwner) return;
